...
###############################################################################
	...
	def mqttPublish(self, data):
		...
		(rc, mid) = self.client.publish(self.mqttTopic, str(data), qos=2, retain=True)
...
###############################################################################
	...
	def calcDoorState(self):
		...
		if self.sensorType["limitOpen"] or self.sensorType["limitClosed"]:
			...
			elif True not in [self.limitStates["open"], self.limitStates["closed"]]:
		...
		if self.sensorType["rotary"] and not limitState:
			doorState = int(
				interp(
					self.rotaryCount, [self.rotaryLimits["closed"], self.rotaryLimits["open"]], [0, 100]
				)
			)
